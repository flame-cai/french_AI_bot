...
const Chat = ({selectedWeek}) => {
  ...
  useEffect(() => {
    if (user) {
      setMessages([{
        role: 'assistant',
        content: ["Salut, je suis Pierre, your practice companion. You can practice your French with me. If you're unsure of how to say something, say it in English and I will show you how to say it in French."]
      }]);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post('https://asia-south1-ppt-tts.cloudfunctions.net/fr-lang-backend/chat', {
  ...
  const insertCharacter = (char) => {
    ...
    // Move the cursor position to after the inserted character
    setTimeout(() => {
        ...
        try {
          ...
          return (
            <p key={idx} className="text-container">
              ...
              <TextToSpeech text={splitArray.map((subArray) => subArray[0]).join(' ')} speed={0.85}/>
            </p>
          );
        } catch (error) {
  ...
  return (
    <div className="chat-container">
      <h1>French Conversation Companion</h1>
      <hr />
          ...
          <div key={index} className={`message-row ${msg.role}-row`}>
            {msg.role === 'assistant' && (
              <div className="profile-picture-container">
                <div className='profile-picture items-center justify-center rounded-full font-bold h-7 w-7 bg-text-200 text-bg-100'>P</div>
              </div>
            )}
    ...
    <div className="message-row assistant-row">
      ...
      <div className="message assistant">
        ...
        <div ref={messageEndRef} /> {/* Dummy div to serve as scroll target */}
      </div>
      <div className="input-container">
        ...
        <div className="umlaut-buttons">
          {['é', 'à', 'è', 'ù', 'â', 'ê', 'î', 'ô', 'û', 'ë', 'ï', 'ö', 'ü', 'ç'].map((char) => (
            <button key={char} onClick={() => insertCharacter(char)}>
              {char}
            </button>
          ))}
        </div>
